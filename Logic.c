...
#ifdef WIN32
#	define mythinline _inline
#else
#	define mythinline inline
#endif
static mythinline char* mythReplace(char* str, int len, int left, int right){
	if (left > (len >> 1)){
		str[left] = 'p';
...
static mythinline int isAtom(char* str, int left, int right){
	switch (right - left - 1){
	case 5:
